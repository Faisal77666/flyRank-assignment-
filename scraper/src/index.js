...
async function fetchWithCache(url, cacheFilename) {
  ...
  // Retry once on timeout or 5xx server errors — never on 404 or 403
  let lastError;
...
async function main() {
  ...
  // TEMP: uncomment the line below to test failure handling with a fake URL
  ...
  for (const url of bookUrls) {
    ...
    try {
      ...
      continue; // one bad page does not stop the run
    }
